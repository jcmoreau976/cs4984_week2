<?php
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    // TODO add validations
    $errors = valid_basic($salesperson);
    
    if(preg_match("/[^0-9-()]/", $salesperson['phone'])){
        $errors[] = "Phone number may only contain digits, dashes, or parentheses";
    }
    if(preg_match("/[^a-zA-Z0-9@._-]/", $salesperson['email'])){
        $errors[] = "Email contains an illegal character";
    }
  ...
  function insert_salesperson($salesperson) {
    ...
    $salesperson = scrub($db, $salesperson);
  ...
  function validate_user($user, $errors=array()) {
    ...
    elseif(preg_match("/[^a-zA-Z0-9_]+/", $user['username'], $illegal=array())){
        $errors[] = "Username may only contain alphanumeric characters or underscores";
    }
    if(preg_match("/[^a-zA-Z0-9@._-]/", $user['email'])){
        $errors[] = "Email contains an illegal character";
    }
    return $errors;
  }
  ...
  function update_user($user) {
    ...
    $esc_sql = mysqli_real_escape_string($sql);
    $result = db_query($db, $sql);
    if($result) {
      return true;
    } else {
